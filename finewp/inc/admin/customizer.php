...
function finewp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('finewp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'finewp'), 'priority' => 10, ));
    endif;
    
    $wp_customize->get_section( 'title_tagline' )->panel = 'finewp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'finewp_main_options_panel';
    ...
    finewp_getting_started($wp_customize);
    finewp_header_options($wp_customize);
    finewp_slider_options($wp_customize);
    finewp_post_options($wp_customize);
    finewp_social_profiles($wp_customize);
    finewp_footer_options($wp_customize);
    finewp_other_options($wp_customize);
    finewp_recomm_plugin_options($wp_customize);
    finewp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'finewp_register_theme_customizer' );

function finewp_customizer_js_scripts() {    
    wp_enqueue_script('finewp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'finewp_customizer_js_scripts' );